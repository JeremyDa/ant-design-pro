...
const CreateForm = Form.create()(props => {
  ...
  const { fId, fName, fSex, fClass } = record;
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="学生id">
      ...
      <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="所属班级">
        {form.getFieldDecorator(
          'fClass',
          // fSelect && { initialValue: `${fSelect}`,
          {
            initialValue: fClass ? `${fClass}` : ``,
          }
        )(
          <Select placeholder="请选择" style={{ width: '100%' }}>
            {T_CLASS
  ...
  getColumns = table => {
    ...
    return [
      ...
      // to update: value翻译成key
      {
        ...
        // ],
        render(val) {
          return <span>{T_CLASS ? T_CLASS.kv[val] : ''}</span>;
